...
typedef struct FilaFlexCircular {
    struct Celula* primeiro, *ultimo;
    int tamanho;
}FilaFlexCircular;

FilaFlexCircular* criarFilaFlexCircular(){
    FilaFlexCircular* new = malloc(sizeof(FilaFlexCircular));
    new->primeiro = malloc(sizeof(Celula));
    new->ultimo = new->primeiro;
    new->ultimo->prox  = NULL;
    new->primeiro->prox  = NULL;
    new->tamanho = 0;
    return new;
}
//----------------------------FUNÇÕES FILA FLEXIVEL ----------------------------------------------------
void mostrar(FilaFlexCircular* fila){
...
void inserir(FilaFlexCircular* fila, Jogador x){
    if(fila->tamanho < FILA_TAMANHO){
        fila->ultimo->prox = criarCelula(x);
        ...
        fila->ultimo = fila->ultimo->prox;
        fila->ultimo->elemento = x;
    }
}
Jogador remover(FilaFlexCircular* fila){
    Jogador resp;
    if(fila->tamanho == 0) {
        printf("FILA VAZIA JA!\n");
        return resp;
    }
    resp = fila->primeiro->prox->elemento;
...
int mediaAltura(FilaFlexCircular* fila){
    int alturas = 0;
    int numJogadores = fila->tamanho;

    Celula* i = fila->primeiro->prox;

    while(i != fila->primeiro)
    {
        alturas += i->elemento.altura;
        i = i->prox;
    }
...
// .....................................PARTE 2......................................
    ...
    FilaFlexCircular* filaCircular = criarFilaFlexCircular();
