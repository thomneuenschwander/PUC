...
int compararJogadores(const void* a, const void* b) {
    ...
    int estadoComparison = strcasecmp(jogadorA->estadoNascimento, jogadorB->estadoNascimento);

    if (estadoComparison == 0) {
        return strcasecmp(jogadorA->nome, jogadorB->nome);
    }
...
void quicksortRec(Jogador* arr, int esq, int dir) {
    ...
    while (i <= j) {
        while (compararJogadores(&arr[i], &pivo) < 0) {
            i++;
        }
        while (compararJogadores(&arr[j], &pivo) > 0) {
            j--;
        }
        if (i <= j) {
            if (i != j) {
...
void quicksort(Jogador* arr, int tamanho) {
    quicksortRec(arr, 0, tamanho - 1);
}
