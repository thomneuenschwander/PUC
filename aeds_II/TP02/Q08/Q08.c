...
//             int j;
//             for(j = i; j >= h && arr[j - h].peso > temp; j -= h){
    ...
    for (gap = n / 2; gap > 0; gap /= 2) {
        for (i = gap; i < n; i++) {
            for (j = i - gap; j >= 0; j -= gap) {
                if (arr[j].peso > arr[j + gap].peso) {
                    temp = arr[j];
                    arr[j] = arr[j + gap];
                    arr[j + gap] = temp;
                } else if (arr[j].peso == arr[j + gap].peso) {
                    if (strcmp(arr[j].nome, arr[j + gap].nome) > 0) {
                        temp = arr[j];
                        arr[j] = arr[j + gap];
                        arr[j + gap] = temp;
                    }
...
int main()
{
    ...
    //FILE *arq = fopen("C:\\Users\\Pichau\\Desktop\\TP02\\Q08\\players.csv", "r");
    ...
    shellSort(jg_entrada, index_jg_entrada);
